...
program
  ...
  .hook("preAction", () => {
    if (program.opts().debug) enableDebug();
  });
...
profile.command("switch")
  ...
  .action((name) => {
    try { profileSwitch(name); } catch (err) { handleError(err); }
  });
...
profile.command("remove")
  ...
  .action((name) => {
    try { profileRemove(name); } catch (err) { handleError(err); }
  });
...
projects.command("attach").description("Attach a project to a transaction")
  ...
  .action(async (options) => {
    if (!options.projectId) { console.error("Error: --project-id is required"); process.exit(1); }
    if (!options.entity) { console.error("Error: --entity is required"); process.exit(1); }
    if (!options.entityId) { console.error("Error: --entity-id is required"); process.exit(1); }
...
customFields.command("create").description("Create a custom field definition")
  ...
  .option("--forms <forms>", "Forms the field appears on. Comma-separated and/or repeatable (--forms invoice,estimate OR --forms invoice --forms estimate). Required for every category. Valid: invoice, estimate, sales-receipt, credit-memo, refund-receipt, sales-order, bill, expense, check, purchase-order, vendor-credit, credit-card-credit.", (val: string, prev: string[] = []) => prev.concat(val.split(",").map((s) => s.trim()).filter(Boolean)))
  .option("--option <value>", "Dropdown option value (repeatable). Only valid with --data-type STRING_LIST.", (val: string, prev: string[] = []) => prev.concat([val]))
...
customFields.command("update").description("Update a custom field definition (rename, activate/deactivate, or add forms)")
  ...
  .option("--active <bool>", "Set active (true) or inactive (false). Use false to soft-delete.", (val: string) => val === "true")
  ...
  .option("--forms <forms>", "Forms the field appears on. Comma-separated and/or repeatable. Server appends to existing list; cannot remove via this flag.", (val: string, prev: string[] = []) => prev.concat(val.split(",").map((s) => s.trim()).filter(Boolean)))
...
webhooks.command("listen").description("Start a local listener to capture webhook events")
  ...
  .action((options) => {
    ...
    webhooksListen({
      ...
      verifierToken,
      events: options.events ? options.events.split(",").map((e: string) => e.trim()) : [],
...
function addFuzzyMatching(parent: Command, prefix: string) {
