...
export async function accountsList(options: ListOptions & { json?: boolean; csv?: boolean }, profile?: string) {
  const accounts = await intuitQueryPaginated("Account", options, profile) as Record<string, unknown>[];
