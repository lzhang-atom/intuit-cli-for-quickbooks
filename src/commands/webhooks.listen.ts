...
interface WebhookListenOptions {
  port: number;
...
export function verifySignature(payload: string, signature: string, verifierToken: string): boolean {
  const hash = crypto
...
export function matchesFilter(type: string, filters: string[]): boolean {
  if (filters.length === 0) return true;
  return filters.some(f => f.toLowerCase() === type.toLowerCase());
}

export function webhooksListen(options: WebhookListenOptions) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    const event: StoredEvent = { timestamp, payload, signatureValid: true };
    storedEvents.push(event);
    saveEvents(storedEvents);

    try {
      const cloudEvents = JSON.parse(payload) as Array<Record<string, unknown>>;
      for (const ce of cloudEvents) {
        const type = (ce.type as string) || "";

        if (!matchesFilter(type, eventFilters)) continue;
