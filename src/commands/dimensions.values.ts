...
export async function dimensionsValues(
  definitionId: string,
  options: { json?: boolean; csv?: boolean },
  profile?: string
) {
  const values: DimValueNode[] = [];
  let after: string | null = null;

  do {
    const data = await graphqlQuery(
      QUERY,
      { first: 100, after, filters: { definitionId, parentId: null } },
      "GetActiveCustomDimensionValues",
      profile
    );
    const result = data.appFoundationsActiveCustomDimensionValues as {
      edges: { node: DimValueNode }[];
      pageInfo: { hasNextPage: boolean; endCursor: string };
    };

    for (const edge of result.edges) values.push(edge.node);
