...
function isAttachableLine(line) {
...
function attachDimensionToLine(line, definitionId, valueId) {
    const existing = line.CustomExtensions || [];
    // Find existing DIMENSION extension, or create a new one
    let dimExt = existing.find(e => e.ExtensionType === "DIMENSION");
    const assocValues = dimExt
        ? [...dimExt.AssociatedValues]
        : [];
    // Replace or add the value for this definition
    const idx = assocValues.findIndex(av => av.Key === definitionId);
    if (idx >= 0)
        assocValues[idx] = { Key: definitionId, Value: valueId };
    else
        assocValues.push({ Key: definitionId, Value: valueId });
    if (dimExt) {
...
export async function dimensionsAttach(options, profile) {
    ...
    if (options.file) {
        ...
        if (!options.definitionId)
            throw new Error("--definition-id is required (or use --file for per-line control)");
        if (!options.valueId)
            throw new Error("--value-id is required (or use --file for per-line control)");
