...
async function fetchField(id, profile) {
...
export async function customFieldsUpdate(id, options, profile) {
    if (options.dataType) {
        throw new Error(`Cannot change --data-type on an existing field. Data type is set at create time and is immutable. ` +
    ...
    if (options.file) {
        ...
        if (options.category) {
            ...
            throw new Error(`Cannot change --category on an existing field. Category is set at create time and is immutable. ` +
        ...
        if (options.transactions && options.transactions.length > 0) {
            ...
            if (existingParents.length === 0) {
                throw new Error(`Field ${id} has no /transactions/Transaction association to update. ` +
            ...
            input.associations = (existing.associations || []).map((a) => {
                if (a.associatedEntity !== "/transactions/Transaction")
                    return a;
                return {
                    ...a,
    ...
    const field = data.appFoundationsUpdateCustomFieldDefinition;
